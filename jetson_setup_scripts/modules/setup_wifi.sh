...
if ! conn_output=$(sudo nmcli device wifi connect "$SSID" password "$PASS" ifname "$IFACE" 2>&1); then
  warn "Wi-Fi への接続に失敗しました: ${conn_output}"
  warn "SSID/PASS/IFACE を確認し、再実行してください"
else
